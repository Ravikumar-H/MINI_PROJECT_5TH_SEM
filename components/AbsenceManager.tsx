...
import { Teacher, TimetableData, User } from '../types';
...
interface AbsenceManagerProps {
    ...
    onAbsenceSubmit: (teacherName: string, day: string, period: number) => void;
    isLoading: boolean;
    user: User;
}

const AbsenceManager: React.FC<AbsenceManagerProps> = ({ teachers, timetable, onAbsenceSubmit, isLoading, user }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (selectedTeacher && selectedDay && selectedPeriod) {
            onAbsenceSubmit(selectedTeacher, selectedDay, selectedPeriod);
        }
    };
        ...
        <div>
            ...
            <p className="text-sm text-gray-600 mb-6">
                {user.role === 'HOD' 
                    ...
                    : "Report a teacher's absence to automatically find a substitute."
                }
            </p>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                    {isLoading ? <LoadingSpinner /> : 'Find Substitute & Notify'}
                </button>
            </form>
        </div>
    );
